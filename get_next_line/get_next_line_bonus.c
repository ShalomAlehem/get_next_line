...
char	*get_next_line(int fd)
{
	...
	static char	*remainer[MAX_FD];
	char		*line;

	if (fd > MAX_FD - 1 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	line = ft_get_line(&remainer[fd]);
	ft_liner(fd, buffer, &line, &remainer[fd]);
	if (remainer[fd] && *remainer[fd] == '\0')
		ft_freemem(&remainer[fd]);
	if (!ft_strlen(remainer[fd]) && !ft_strlen(line))
	{
		ft_freemem(&line);
		return (NULL);
	}
